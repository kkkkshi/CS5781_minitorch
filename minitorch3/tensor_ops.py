...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for i in range(len(out)):
            to_index(i, out_shape, out_index)  # find/change out_index
            o = index_to_position(out_index, out_strides)
            broadcast_index(
            ...
            j = index_to_position(
                a_index, a_strides
            )  # find that position in a_storage corresponding to a_index
            broadcast_index(
                out_index, out_shape, b_shape, b_index
            )  # find/change b_index
            k = index_to_position(
                b_index, b_strides
            )  # find that position in b_storage corresponding to b_index
    ...
    def _reduce(
        ...
        for i in range(len(out)):
            ...
            for s in range(a_shape[reduce_dim]):
                to_index(s, reduce_shape, a_index)
