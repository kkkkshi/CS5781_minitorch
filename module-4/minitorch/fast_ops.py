    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        for i in prange(len(out)):
            ...
            t = out[index_to_position(out_idx, out_strides)]
            for _ in range(a_shape[reduce_dim]):
                t = fn(t, a_storage[a_pos])
...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    for i0 in prange(out_shape[0]):
        for i1 in prange(out_shape[1]):
            for i2 in prange(out_shape[2]):
                a_i = i0 * a_batch_stride + i1 * a_strides[1]
                b_i = i0 * b_batch_stride + i2 * b_strides[2]
                t = 0.0
                for _ in range(a_shape[2]):
                    t += a_storage[a_i] * b_storage[b_i]
                    a_i += a_strides[2]
                    b_i += b_strides[1]
                out[i0 * out_strides[0] + i1 * out_strides[1] + i2 * out_strides[2]] = t
